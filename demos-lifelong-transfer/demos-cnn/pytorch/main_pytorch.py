...
Model_target_list = [DecisionLevelMaxPooling, Vggish, ResNet]  # [DecisionLevelMaxPooling, Vggish, ResNet] [ResNet, Vggish, DecisionLevelMaxPooling] 
Model_attacker_folder = 'CnnAtrous' 
Model_target_folder_list = ['models-basecnn', 'models-vgg16', 'models-resnet50']  #['models-basecnn', 'models-vgg16', 'models-resnet50'] ['models-resnet50', 'models-vgg16', 'models-basecnn']
Model_num = 3
...
def train(args):
	...
	if not model_num == 0:
	    ...
	    models_attacker_path_pre = os.path.join(models_attacker_dir, 'md_{}_iters.tar'.format(ite_base*(model_num)))
            if validation:
	...
	if not model_num == 0:
		...
		ite_save = 1000
            if iteration % ite_save == 0 and iteration > 0:
                ...
                save_out_path = os.path.join(models_attacker_dir, 'md_{}_iters.tar'.format(ite_base*(model_num+1)))
...
def inference_validation_data(args):
	...
	for target_num in range(0, Model_num):
            ...
            logging.info('model_num: {}\ttarget_num: {}'.format(model_num, target_num))
	...
	##########################################################################
		...
		heatmaps_info = []
			...
			heatmaps_noise.append(noise_heatmap[i])
            		heatmaps_info.append([audio_names[i], pred_num, pred_num_adv, outputs[i][pred_num], outputs_adv[i][pred_num_adv]])
		...
		if not os.path.exists(os.path.join(workspace, 'models', folder_name)):
        	...
        	np.save(os.path.join(workspace, 'models', folder_name, "model_num"+str(model_num)+"target_num"+str(target_num)+"-"+file_name+"-heatmap.npy"),heatmaps_input)##############
        	np.save(os.path.join(workspace, 'models', folder_name, "model_num"+str(model_num)+"target_num"+str(target_num)+"-"+file_name+"-heatmap_adv.npy"),heatmaps_output)##############
        	np.save(os.path.join(workspace, 'models', folder_name, "model_num"+str(model_num)+"target_num"+str(target_num)+"-"+file_name+"-heatmap_noise.npy"),heatmaps_noise)##############
        	np.save(os.path.join(workspace, 'models', folder_name, "model_num"+str(model_num)+"target_num"+str(target_num)+"-"+file_name+"-info.npy"), heatmaps_info)#########                 
	###########################################################################    
